...
import {
  StreamflowRealReconcileEligibilityError,
  StreamflowRealReconcileExecutionGuardError,
  reconcileRealStreamflowScheduleForVestingPosition,
} from "@/lib/vesting/streamflow-reconcile-real";
...
export async function POST(request: Request) {
  try {
    ...
    await enforceRateLimit({
      key: `internal-admin-streamflow-reconcile-real:${clientIp}`,
      limit: 10,
      windowMs: 60_000,
    });
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: "Invalid real Streamflow reconciliation request." },
        { status: 400 },
      );
    }

    if (error instanceof StreamflowRealReconcileExecutionGuardError) {
      return NextResponse.json({ error: error.message }, { status: 409 });
    }

    if (error instanceof StreamflowRealReconcileEligibilityError) {
      return NextResponse.json({ error: error.message }, { status: 422 });
    }

    const message =
      error instanceof Error ? error.message : "Unable to reconcile real Streamflow schedule.";
